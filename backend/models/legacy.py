"""
Legacy Waypoint dataclass — CUP format model.
Kept separate from the SQLAlchemy models to avoid naming conflicts.
"""
import re
...
class Waypoint:
    ...
    def to_cup_string(self):
        ...
        return (f'"{self.name}","{self.code}","{self.country}",'
                f'{lat_str},{lon_str},{self.elevation}m,{self.style},'
                f'{rwdir},{rwlen},{rwwidth},"{self.frequency}","{self.description}"')
